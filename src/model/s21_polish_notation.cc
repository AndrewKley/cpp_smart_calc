 ...
 */
int s21::PolishNotation::get_operator(std::string str, size_t &i) noexcept {
  size_t len = str.length();
  ...
  if (ch == 0 && i + MAX_OPERATOR_LEN - 1 < len) {
    ...
    if (ch == 0 && i + MAX_OPERATOR_LEN < len) ch = get_func_operator(str, i);
  }
  return ch;
}

int s21::PolishNotation::get_func_operator(std::string &str,
                                           size_t &i) noexcept {
  int ch = 0;
 ...
 */
int s21::PolishNotation::get_cos_sin_tan(std::string str, size_t &i) noexcept {
  char ch = 0;
...
bool s21::PolishNotation::is_dot(char c) const noexcept { return c == '.'; }
...
bool s21::PolishNotation::is_heigh(char c) const noexcept {
  bool tmp = false;
...
bool s21::PolishNotation::is_equal(const std::stack<char> &st,
...
int s21::PolishNotation::is_less(const std::stack<char> &st,
                                 char c) const noexcept {
...
s21::Status s21::PolishNotation::take_everything_from_stack(
    std::stack<char> &st, std::string &res) noexcept {
  while (st.size()) {
    char ch = st.top();
    st.pop();
    if (ch != OPEN && ch != CLOSE) {
