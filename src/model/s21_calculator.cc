 ...
 */
s21::Status s21::Calculator::calculate_converted_expression(
    std::string &str, double &x, double &res) noexcept {
  Status status = OK;
...
double s21::Calculator::switch_unary(double num,
...
s21::Status s21::Calculator::calculate_double(std::stack<double> &st,
  ...
  if (st.size() >= 2) {
    ...
    else if ((num_second < 0.) && operation == DEG)
      status = ERROR;
...
int s21::Calculator::take_operation(char ch) const noexcept {
  int is_operation = 0;
...
int s21::Calculator::is_unary_operator(char operation) const noexcept {
  int is_unary = 0;
...
s21::Status s21::Calculator::is_nan(double num, char operation) const noexcept {
  Status is_nan = OK;
...
double s21::Calculator::s21_atof(const std::string &str, size_t &i) noexcept {
  double num = 0;
  int exp = 0, is_exp = 0;
  size_t j = i;
